...
const SHOPEE_URL_REGEX =
  /https:\/\/shopee\.vn\/.*?-i\.(\d+)\.(\d+)|https:\/\/shopee\.vn\/product\/(\d+)\/(\d+)/i;
...
export function ImportProductsModal({
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="w-[95vw] max-w-lg p-4 sm:p-6 rounded-lg">
        <DialogHeader>
          <DialogTitle>Import Sản Phẩm Shopee</DialogTitle>
          <DialogDescription className="text-sm">
            Nhập danh sách link sản phẩm Shopee để tự động import shopId và
            productId vào hệ thống.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="border-2 border-dashed border-muted-foreground/25 rounded-xl aspect-video flex flex-col items-center justify-center p-6 bg-muted/5 transition-colors hover:bg-muted/10">
              ...
              <div className="text-center space-y-2">
                <p className="text-sm font-medium">
                  Kéo thả file .xlsx hoặc click để chọn
                </p>
                <p className="text-xs text-muted-foreground">
                  Hệ thống sẽ lấy dữ liệu từ Cột A (cột đầu tiên) của Sheet 1
                </p>
              </div>
              <input
                ref={fileInputRef}
